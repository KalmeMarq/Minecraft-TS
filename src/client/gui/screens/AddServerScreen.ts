...
export default class AddServerScreen extends ScreenP {
  ...
  public textServerAdress: string = ''
  public buttonAddServer: Button | any
  public field_243290_a = new TranslationTextComponent("addServer.enterName").get();
  public field_243291_b = new TranslationTextComponent("addServer.enterIp").get();
    ...
    this.buttonAddServer = this.addButton(new Button(this.width / 2 - 100, this.height / 4 + 96 + 18, 200, 20, new TranslationTextComponent("addServer.add").get(), () => {
      ...
      if(serverIp !== '') {
        async function getServerData() {
          ...
          if(serversList.findIndex((serv: any) => serv.ip === data.ip) === -1) {
