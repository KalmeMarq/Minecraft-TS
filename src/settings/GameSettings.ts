...
export default class GameSettings {
  ...
  public async load (): Promise<void> {
    if (!(await Util.fileExists(this.optionsFile))) {
      return
    }

    const optionsData = Buffer.from((await Util.getFile(this.optionsFile)).data).toString('utf8')
    ...
    console.log(options.keys(), options.values())
  ...
  public save (): void {
    Util.createFile(this.optionsFile, () => {
      let data = ''
