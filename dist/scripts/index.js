...
import { CharacterRenderer } from "./client/gui/FontRenderer.js";
let ticks = true;
export const setTicks = (state) => {
    ticks = state;
...
export let characterRenderers = {};
export let addCharacterRenderer = (color, char) => {
...
class Minecraft {
    ...
    static getVersionType() {
        return versionType;
    }
    static isDemo() {
        return isDemo;
    }
    static isModdedClient() {
...
const initialize = async () => {
    async function fetchAllData() {
        ...
        langNames.forEach(async (name) => await JSONUtils.getJSONFile(`./${rootloc}/lang/${name}.json`, (data) => Resources.languages.push({ code: name, data: data })));
        await JSONUtils.getTextFile(`./${rootloc}/texts/credits.txt`, (data) => data.split(/\r?\n/).forEach((line) => Resources.texts.credits.push(line)));
        await JSONUtils.getTextFile(`./${rootloc}/texts/end.txt`, (data) => data.split(/\r?\n/).forEach((line) => Resources.texts.end.push(line)));
        await JSONUtils.getTextFile(`./${rootloc}/texts/splashes.txt`, (data) => data.split(/\r?\n/).forEach((line) => Resources.texts.splashes.push(line)));
