...
export default class Button {
    constructor(x, y, width, height, text, clickAction) {
        ...
        if (this.isActive)
            ...
            this.drawBtnBg({ x: 0, y: 46 }, image, btnBg, ctx);
        const mouseEnterEv = () => {
            if (this.isActive) {
        ...
        this.btn.addEventListener('focusout', () => {
            if (this.isActive) {
                ...
                this.drawBtnBg({ x: 0, y: 46 }, widgetsImg, btnBg, ctx);
        });
    ...
    active(state) {
        ...
        if (this.isActive)
            this.drawBtnBg({ x: 0, y: 66 }, widgetsImg, this.btn.firstElementChild, this.btn.firstElementChild.getContext('2d'));
        else
            this.drawBtnBg({ x: 0, y: 46 }, widgetsImg, this.btn.firstElementChild, this.btn.firstElementChild.getContext('2d'));
        this.render();
    }
}
